        ...
        try {
            ...
            foreach ($conditions as $condition) {
                ...
                if (isset($this->options['withoutGlobalScopes']) && $this->options['withoutGlobalScopes']) {
                    ...
                    $collection = $query->withoutGlobalScopes($scopesToExclude)->where('created_by_id', $this->options['user_id'])->get();
                } else {
                    $collection = $query->where('created_by_id', $this->options['user_id'])->get();
                }

                if ($this->options['report_type'] != 'group_by_relationship') {
                    $collection->where($this->options['group_by_field'], '!=', '')->where('created_by_id', $this->options['user_id']);
                }
